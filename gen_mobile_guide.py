...
nfc_svg = '''<svg viewBox="0 0 24 24" fill="#C8A96A" xmlns="http://www.w3.org/2000/svg" style="width:32px;height:32px;">
...
steps = [
    ...
    ("선암사의 자연 소리를 감상하세요",  "Enjoy the natural soundscape of Seonamsa",        svg4,
     "선암사에서 직접 녹음한 음원입니다.<br>산사의 소리풍경에 귀를 기울여 보세요."),
]
...
.dbar {{
  ...
  background:repeating-linear-gradient(90deg,
    #C8A96A 0,#C8A96A 18px,
    #2D6B58 18px,#2D6B58 36px,
    #D9C5B2 36px,#D9C5B2 54px,
    #2D6B58 54px,#2D6B58 72px);
  opacity:.85;
}}
...
.bgc {{
  ...
  background:#C8A96A; opacity:.05; pointer-events:none;
}}
...
.bt-sub {{
  ...
  color:#E2C98A; opacity:.9;
}}
.bt-ko {{
  font-family:'Nanum Myeongjo',serif;
  font-size:22px; letter-spacing:.18em;
  color:#D9C5B2; opacity:.72;
}}
...
.nfc-txt {{
  ...
  letter-spacing:.22em; color:#E2C98A;
...
.sec-label-ko {{
  ...
  color:#E2C98A; letter-spacing:.1em; white-space:nowrap;
}}
.sec-label-en {{
  font-family:'Montserrat',sans-serif;
  font-size:20px; font-weight:600;
  color:#E2C98A; letter-spacing:.18em;
...
.sko {{
  ...
  color:#EDE3D8; line-height:1.3;
}}
.sen {{
  font-family:'Montserrat',sans-serif;
  font-size:24px; color:#D9C5B2;
  opacity:.55; line-height:1.3; margin-top:6px;
}}
.snote {{
  font-family:'Nanum Myeongjo',serif;
  font-size:22px; color:#E2C98A;
...
.footer {{
  flex-shrink:0;
  border-top:1px solid rgba(200,169,106,.28);
  padding-top:36px;
...
.ubadge {{
  ...
  padding:14px 48px;
  border-width:2px; border-style:solid; border-color:#C8A96A;
  border-radius:999px;
  font-family:'Montserrat',sans-serif;
...
.obadge {{
  display:inline-block;
  padding:10px 28px;
  border-width:1.5px; border-style:solid; border-color:rgba(200,169,106,.4);
  border-radius:999px;
  font-family:'Noto Sans KR',sans-serif;
...
.copy {{
  font-family:'Montserrat',sans-serif;
  font-size:16px; color:#D9C5B2; opacity:.3;
  letter-spacing:.04em;
}}
</style>
</head>
...
out = '/home/user/webapp/seonamsa/nfc-guide-mobile.html'
